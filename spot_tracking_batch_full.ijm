...
for (f=0;f<lengthOf(dir_list);f++)
{
	if (substring(dir_list[f],lengthOf(dir_list[f])-1,lengthOf(dir_list[f]))=="/")
	{
		dir_ori=dir+dir_list[f];
		//dir_ori=getDirectory("Analysis Directory");
orifi=getFileList(dir_ori);
ori_files=lengthOf(orifi);
if (File.exists(dir_ori+"/Rois/")==1){
...
{
	setSlice(frame[k]+1);
	fillOval(x_pos[k]-2, y_pos[k]-2, 5, 5);
}
...
coloc=getTitle();imageCalculator("Subtract create stack", "fluo1","fluo2");
cdc13_only=getTitle();

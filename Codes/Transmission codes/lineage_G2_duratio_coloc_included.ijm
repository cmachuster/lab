...
//filling coloc array
pre_coloc=newArray(lin);
coloc_dau=newArray(lin);
coloc_mo=newArray(lin);
Array.fill(pre_coloc, 100);
...
for (bb=0;bb<lin;bb++)
{
	...
	pre_coloc[bb]=getResult("cell_"+mo[bb],frame[bb]-2);
...
for (ww=0;ww<lin;ww++)
{
	...
	if (pre_coloc[ww]>0 && coloc_dau[ww]>0 && coloc_mo[ww]>0 && pre_coloc[ww]<100)

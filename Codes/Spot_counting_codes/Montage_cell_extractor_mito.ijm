...
//Exctract Middle plane RFP
mito_dir=destination2+"/mito_midplane/";
File.makeDirectory(mito_dir);

run("Duplicate...", "duplicate channels="+3+" slices="+frame);
run("Montage to Stack...", "images_per_row="+montage+" images_per_column="+montage+" border=0");
run("Image Sequence... ", "format=TIFF name=mito save="+mito_dir+"mito0000.tif");
close();close();

...
for (t=0;t<lin;t++)
{
	...
	if (limit<framer-3)
	{
		if (getResult("cell_"+mo[t],limit+1)>0) {spot_mo[t]="t2";}
		if (getResult("cell_"+dau[t],limit+1)>0) {spot_dau[t]="t2";}
		if (getResult("cell_"+mo[t],limit+2)>0) {spot_mo[t]="t3";}
		if (getResult("cell_"+dau[t],limit+2)>0) {spot_dau[t]="t3";}
		if (getResult("cell_"+mo[t],limit+1)==0) {spot_mo[t]="none";}
		if (getResult("cell_"+dau[t],limit+1)==0) {spot_dau[t]="none";}
		if (getResult("cell_"+mo[t],limit+2)==0) {spot_mo[t]="none";}
		if (getResult("cell_"+dau[t],limit+2)==0) {spot_dau[t]="none";}
	}
	else {spot_mo[t]="not_counted"; spot_dau[t]="not_counted";}
}
